...
3. Click the **Code** button near the top-right, then click **Download ZIP** — or, from the project folder view, click **⋯ (More options) > Download directory**.
...
The project must be compiled before Javelin can analyze it. For **Maven projects** (Csv, Gson, JacksonDatabind, Jsoup):

- Use the IntelliJ build shortcut: **Build > Build Project** (`Ctrl+F9`)
- Or open the terminal inside IntelliJ and run:
  ```bash
